/*-------------------------------------------------------
 * This file was generated by JParser
 *
 * Do not make changes to this file
 *-------------------------------------------------------*/

package gen.c.imgui.extension.imlayout;

import com.github.xpenatan.jParser.api.NativeEnum;

public enum ImOrientation implements NativeEnum<ImOrientation> {

    CUSTOM(0), NONE(NONE_NATIVE()), HORIZONTAL(HORIZONTAL_NATIVE()), VERTICAL(VERTICAL_NATIVE());

    private int value;

    private ImOrientation(int value) {
        this.value = value;
    }

    public int getValue() {
        return value;
    }

    public ImOrientation setValue(int value) {
        if (this != CUSTOM) {
            throw new RuntimeException("Cannot change none CUSTOM value");
        }
        this.value = value;
        return this;
    }

    public ImOrientation getCustom() {
        return CUSTOM;
    }

    @org.teavm.interop.Import(name = "imgui_extension_imlayout_imorientation_none_native")
    private static native int NONE_NATIVE();

    @org.teavm.interop.Import(name = "imgui_extension_imlayout_imorientation_horizontal_native")
    private static native int HORIZONTAL_NATIVE();

    @org.teavm.interop.Import(name = "imgui_extension_imlayout_imorientation_vertical_native")
    private static native int VERTICAL_NATIVE();
}
