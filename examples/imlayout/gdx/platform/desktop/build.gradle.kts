...
val extensionRuntimeProject = ":extensions:imlayout:imlayout-c"
val extensionBuilderProject = ":extensions:imlayout:imlayout-build"
val teaVMBuilderMainClass = "imgui.example.imlayout.gdx.c.ImLayoutTeaVMCBuilder"
...
dependencies {
    ...
    if(examplesUseMavenArtifacts) {
        ...
        implementation(libs.jImGuiImlayoutC)
        compileOnly(libs.jImGuiImguiCore)
        compileOnly(libs.jImGuiImlayoutCore)
        nativeRuntimeClasspath(libs.jImGuiImlayoutC)
    }
    else {
...
tasks.register<JavaExec>("imgui_imlayout_c_gdx_desktop_gl_generate") {
    group = "example-desktop-c"
    description = "Generate the ImLayout GDX OpenGL desktop example with TeaVM C."
    configureTeaVM("generate")
}

tasks.register<JavaExec>("imgui_imlayout_c_gdx_desktop_gl_build") {
    group = "example-desktop-c"
    description = "Build the ImLayout GDX OpenGL desktop example with TeaVM C."
    configureTeaVM("build")
}

tasks.register<JavaExec>("imgui_imlayout_c_gdx_desktop_gl_run") {
    group = "example-desktop-c"
    description = "Build and run the ImLayout GDX OpenGL desktop example with TeaVM C."
    configureTeaVM("run")
}
