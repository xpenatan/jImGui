...
val extensionBuilderProject = ":extensions:imgui-node-editor:nodeeditor-build"
val teaVMBuilderMainClass = "imgui.example.nodeeditor.gdx.c.NodeEditorTeaVMCBuilder"
...
tasks.register<JavaExec>("imgui_nodeeditor_c_gdx_desktop_gl_generate") {
    group = "example-desktop-c"
    description = "Generate the imgui-node-editor GDX OpenGL desktop example with TeaVM C."
    configureTeaVM("generate")
}

tasks.register<JavaExec>("imgui_nodeeditor_c_gdx_desktop_gl_build") {
    group = "example-desktop-c"
    description = "Build the imgui-node-editor GDX OpenGL desktop example with TeaVM C."
    configureTeaVM("build")
}

tasks.register<JavaExec>("imgui_nodeeditor_c_gdx_desktop_gl_run") {
    group = "example-desktop-c"
    description = "Build and run the imgui-node-editor GDX OpenGL desktop example with TeaVM C."
    configureTeaVM("run")
}
