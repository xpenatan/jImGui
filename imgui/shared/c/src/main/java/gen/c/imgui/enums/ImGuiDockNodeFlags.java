/*-------------------------------------------------------
 * This file was generated by JParser
 *
 * Do not make changes to this file
 *-------------------------------------------------------*/

package gen.c.imgui.enums;

import com.github.xpenatan.jParser.api.NativeEnum;

public enum ImGuiDockNodeFlags implements NativeEnum<ImGuiDockNodeFlags> {

    CUSTOM(0),
    None(ImGuiDockNodeFlags_None_NATIVE()),
    KeepAliveOnly(ImGuiDockNodeFlags_KeepAliveOnly_NATIVE()),
    NoDockingOverCentralNode(ImGuiDockNodeFlags_NoDockingOverCentralNode_NATIVE()),
    PassthruCentralNode(ImGuiDockNodeFlags_PassthruCentralNode_NATIVE()),
    NoDockingSplit(ImGuiDockNodeFlags_NoDockingSplit_NATIVE()),
    NoResize(ImGuiDockNodeFlags_NoResize_NATIVE()),
    AutoHideTabBar(ImGuiDockNodeFlags_AutoHideTabBar_NATIVE()),
    NoUndocking(ImGuiDockNodeFlags_NoUndocking_NATIVE());

    private int value;

    private ImGuiDockNodeFlags(int value) {
        this.value = value;
    }

    public int getValue() {
        return value;
    }

    public ImGuiDockNodeFlags setValue(int value) {
        if (this != CUSTOM) {
            throw new RuntimeException("Cannot change none CUSTOM value");
        }
        this.value = value;
        return this;
    }

    public ImGuiDockNodeFlags getCustom() {
        return CUSTOM;
    }

    @org.teavm.interop.Import(name = "imgui_enums_imguidocknodeflags_imguidocknodeflags_none_native")
    private static native int ImGuiDockNodeFlags_None_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguidocknodeflags_imguidocknodeflags_keepaliveonly_native")
    private static native int ImGuiDockNodeFlags_KeepAliveOnly_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguidocknodeflags_imguidocknodeflags_nodockingovercentralnode_native")
    private static native int ImGuiDockNodeFlags_NoDockingOverCentralNode_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguidocknodeflags_imguidocknodeflags_passthrucentralnode_native")
    private static native int ImGuiDockNodeFlags_PassthruCentralNode_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguidocknodeflags_imguidocknodeflags_nodockingsplit_native")
    private static native int ImGuiDockNodeFlags_NoDockingSplit_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguidocknodeflags_imguidocknodeflags_noresize_native")
    private static native int ImGuiDockNodeFlags_NoResize_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguidocknodeflags_imguidocknodeflags_autohidetabbar_native")
    private static native int ImGuiDockNodeFlags_AutoHideTabBar_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguidocknodeflags_imguidocknodeflags_noundocking_native")
    private static native int ImGuiDockNodeFlags_NoUndocking_NATIVE();
}
