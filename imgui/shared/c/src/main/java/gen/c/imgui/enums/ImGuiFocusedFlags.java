/*-------------------------------------------------------
 * This file was generated by JParser
 *
 * Do not make changes to this file
 *-------------------------------------------------------*/

package gen.c.imgui.enums;

import com.github.xpenatan.jParser.api.NativeEnum;

public enum ImGuiFocusedFlags implements NativeEnum<ImGuiFocusedFlags> {

    CUSTOM(0),
    None(ImGuiFocusedFlags_None_NATIVE()),
    ChildWindows(ImGuiFocusedFlags_ChildWindows_NATIVE()),
    RootWindow(ImGuiFocusedFlags_RootWindow_NATIVE()),
    AnyWindow(ImGuiFocusedFlags_AnyWindow_NATIVE()),
    NoPopupHierarchy(ImGuiFocusedFlags_NoPopupHierarchy_NATIVE()),
    DockHierarchy(ImGuiFocusedFlags_DockHierarchy_NATIVE()),
    RootAndChildWindows(ImGuiFocusedFlags_RootAndChildWindows_NATIVE());

    private int value;

    private ImGuiFocusedFlags(int value) {
        this.value = value;
    }

    public int getValue() {
        return value;
    }

    public ImGuiFocusedFlags setValue(int value) {
        if (this != CUSTOM) {
            throw new RuntimeException("Cannot change none CUSTOM value");
        }
        this.value = value;
        return this;
    }

    public ImGuiFocusedFlags getCustom() {
        return CUSTOM;
    }

    @org.teavm.interop.Import(name = "imgui_enums_imguifocusedflags_imguifocusedflags_none_native")
    private static native int ImGuiFocusedFlags_None_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguifocusedflags_imguifocusedflags_childwindows_native")
    private static native int ImGuiFocusedFlags_ChildWindows_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguifocusedflags_imguifocusedflags_rootwindow_native")
    private static native int ImGuiFocusedFlags_RootWindow_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguifocusedflags_imguifocusedflags_anywindow_native")
    private static native int ImGuiFocusedFlags_AnyWindow_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguifocusedflags_imguifocusedflags_nopopuphierarchy_native")
    private static native int ImGuiFocusedFlags_NoPopupHierarchy_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguifocusedflags_imguifocusedflags_dockhierarchy_native")
    private static native int ImGuiFocusedFlags_DockHierarchy_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguifocusedflags_imguifocusedflags_rootandchildwindows_native")
    private static native int ImGuiFocusedFlags_RootAndChildWindows_NATIVE();
}
