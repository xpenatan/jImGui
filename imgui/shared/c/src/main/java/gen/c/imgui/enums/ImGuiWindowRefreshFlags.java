/*-------------------------------------------------------
 * This file was generated by JParser
 *
 * Do not make changes to this file
 *-------------------------------------------------------*/

package gen.c.imgui.enums;

import com.github.xpenatan.jParser.api.NativeEnum;

public enum ImGuiWindowRefreshFlags implements NativeEnum<ImGuiWindowRefreshFlags> {

    CUSTOM(0), None(ImGuiWindowRefreshFlags_None_NATIVE()), TryToAvoidRefresh(ImGuiWindowRefreshFlags_TryToAvoidRefresh_NATIVE()), RefreshOnHover(ImGuiWindowRefreshFlags_RefreshOnHover_NATIVE()), RefreshOnFocus(ImGuiWindowRefreshFlags_RefreshOnFocus_NATIVE());

    private int value;

    private ImGuiWindowRefreshFlags(int value) {
        this.value = value;
    }

    public int getValue() {
        return value;
    }

    public ImGuiWindowRefreshFlags setValue(int value) {
        if (this != CUSTOM) {
            throw new RuntimeException("Cannot change none CUSTOM value");
        }
        this.value = value;
        return this;
    }

    public ImGuiWindowRefreshFlags getCustom() {
        return CUSTOM;
    }

    @org.teavm.interop.Import(name = "imgui_enums_imguiwindowrefreshflags_imguiwindowrefreshflags_none_native")
    private static native int ImGuiWindowRefreshFlags_None_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguiwindowrefreshflags_imguiwindowrefreshflags_trytoavoidrefresh_native")
    private static native int ImGuiWindowRefreshFlags_TryToAvoidRefresh_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguiwindowrefreshflags_imguiwindowrefreshflags_refreshonhover_native")
    private static native int ImGuiWindowRefreshFlags_RefreshOnHover_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imguiwindowrefreshflags_imguiwindowrefreshflags_refreshonfocus_native")
    private static native int ImGuiWindowRefreshFlags_RefreshOnFocus_NATIVE();
}
