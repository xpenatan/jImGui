/*-------------------------------------------------------
 * This file was generated by JParser
 *
 * Do not make changes to this file
 *-------------------------------------------------------*/

package gen.c.imgui.enums;

import com.github.xpenatan.jParser.api.NativeEnum;

public enum ImFontFlags implements NativeEnum<ImFontFlags> {

    CUSTOM(0),
    None(ImFontFlags_None_NATIVE()),
    NoLoadError(ImFontFlags_NoLoadError_NATIVE()),
    NoLoadGlyphs(ImFontFlags_NoLoadGlyphs_NATIVE()),
    LockBakedSizes(ImFontFlags_LockBakedSizes_NATIVE()),
    ImplicitRefSize(ImFontFlags_ImplicitRefSize_NATIVE());

    private int value;

    private ImFontFlags(int value) {
        this.value = value;
    }

    public int getValue() {
        return value;
    }

    public ImFontFlags setValue(int value) {
        if (this != CUSTOM) {
            throw new RuntimeException("Cannot change none CUSTOM value");
        }
        this.value = value;
        return this;
    }

    public ImFontFlags getCustom() {
        return CUSTOM;
    }

    @org.teavm.interop.Import(name = "imgui_enums_imfontflags_imfontflags_none_native")
    private static native int ImFontFlags_None_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imfontflags_imfontflags_noloaderror_native")
    private static native int ImFontFlags_NoLoadError_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imfontflags_imfontflags_noloadglyphs_native")
    private static native int ImFontFlags_NoLoadGlyphs_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imfontflags_imfontflags_lockbakedsizes_native")
    private static native int ImFontFlags_LockBakedSizes_NATIVE();

    @org.teavm.interop.Import(name = "imgui_enums_imfontflags_imfontflags_implicitrefsize_native")
    private static native int ImFontFlags_ImplicitRefSize_NATIVE();
}
